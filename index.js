...
const dataReqLink = 'https://card.wb.ru/cards/v1/detail?appType=1&curr=rub&dest=123585704&spp=27&nm='; // путь для получения полных сведений по номеру артикула, в т.ч. и остатков, размеров, вариантов (подставить номер артикула в конце)
...
async function getArticles() {
    ...
    const colors = await page.evaluate(() => {
        return Array.from(document.querySelectorAll(".slider-color > .sw-slider-colorpicker > .swiper-container > .swiper-wrapper > .j-color > .img-plug")).map((item) => item.getAttribute('href').replace(/https:\/\/www.wildberries.ru\/catalog\//, '').replace(/\/detail.aspx/, ''))
    });
